...
namespace JTobey_PerlCall
{
  ...
  static inline SV* undef () { dTHX; return &PL_sv_undef; }
  Scalar::Scalar () : imp (undef ()) {}
  ...
  Scalar::operator= (const Scalar& o)
  {
    ...
    if (SvREADONLY (imp))
      imp = Perl_newSVsv (o.imp);
    else
      Perl_sv_setsv (imp, o.imp);
		       ...
		       Context cx = SCALAR) const
  {
    ...
    {
      dTHX;
      if (! perl_get_cv ("JTobey::PerlCall::call_method", 0))
