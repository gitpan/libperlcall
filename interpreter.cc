...
#define JTOBEY_PERLCALL_INTERPRETER_PRIVATE			\
...
namespace JTobey_PerlCall
{
  ...
  my_xs_init (pTHXo)
  {
    ...
    define_sub ("JTobey::PerlCall", "my_warner", my_warner);
    Hashref ("SIG") .store ("__WARN__", "JTobey::PerlCall::my_warner");
    // XXX Perl doesn't assign early enough?!
    if (PL_warnhook)
      SvREFCNT_dec (PL_warnhook);
    PL_warnhook = Perl_newSVpv ("JTobey::PerlCall::my_warner", 0);
  }
		     ...
		     const char* const * envp)
  {
    ...
    // error.  XXX
    eval_string
      ("sub JTobey::PerlCall::call_method {	\
            my ($meth, $obj, $args) = @_;	\
            $obj->$meth (@$args);		\
        }");
  ...
  {
    static const char* argv[] = { "PerlCall", "-e0" };
  ...
  XS (Interpreter::xs_entry_hashref)
  {
    ...
    for (int i = 1; i < items - 1; i += 2)
      {
	SvREFCNT_inc (ST (i + 1));  // XXX stupid perl API
	Perl_hv_store_ent (args, ST (i), ST (i + 1), 0);
      }
